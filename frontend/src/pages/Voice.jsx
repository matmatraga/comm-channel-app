...
const VoicePage = () => {
  ...
  const fetchCallHistory = async () => {
    try {
      const res = await axios.get('https://omni-channel-app.onrender.com/api/voice/history');
      setCalls(res.data);
    } catch (err) {
    ...
    const makeCall = async (e) => {
        ...
        try {
            ...
            await axios.post('https://omni-channel-app.onrender.com/api/voice/call', { to: identity });
            setPhone('');
            fetchCallHistory();
        } catch (err) {
    ...
    try {
        ...
        const res = await axios.get(`https://omni-channel-app.onrender.com/api/voice/token?identity=${identity}`);
        ...
        console.error('❌ [INIT DEVICE ERROR]', err.response?.data || err.message);
    }
};
  ...
  return (
    <div className="container mt-4">
      ...
      <ul className="list-group">
        {calls.map((call) => (
          <li className="list-group-item d-flex justify-content-between align-items-center" key={call.callSid}>
            <div>
              <strong>{call.direction === 'inbound' ? 'Incoming' : 'Outgoing'}:</strong> {call.from} ➡️ {call.to}
              <br />
              <small>Status: {call.status} | Duration: {call.duration || 0}s</small>
            </div>
